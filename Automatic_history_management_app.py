...
"""
Önceki örneklerde, mesajları zincire (chain) manuel aktardık. 
Bu yöntem tamamen kabul edilebilir olsa da, yeni mesajların harici olarak yönetilmesini gerektirir. 
LangChain, bu süreci otomatik olarak yönetebilen ve 
LCEL zincirleri için tasarlanmış RunnableWithMessageHistory adlı bir sarıcı (wrapper) içerir.


Sohbet geçmişinden sonra bir HumanMessage şablonunu dolduran son bir (input) alacak şekilde ayarlanır. 
Bu, tüm mesajlar yerine, 
mevcut mesajlardan ÖNCEKİ tüm mesajları içeren bir "chat_history" parametresi gelir("messages" yerine)

RunnableWithMessageHistory sınıfının zincirimizi sarmasına ve 
bu giriş değişkenini sohbet geçmişine ekleme işini yapmasına izin vereceğiz.
"""
from langchain_core.runnables.history import RunnableWithMessageHistory

"""
RunnableWithMessageHistory: Bu sınıf, sarmak istediğimiz zincirin yanı sıra birkaç ek PARAMETRE alır.

lambda session_id: demo_ephemeral_chat_history_for_chain fonksiyonu, 
verilen bir oturum kimliği (session_id) için bir mesaj geçmişi döndürür. 
Bu, zincirinizin aynı anda birden fazla kullanıcıyı işlemesine olanak tanır.

input_messages_key: Girdinin hangi kısmının takip edilip sohbet geçmişinde saklanacağını belirtir(in that case used "input")

history_messages_key:Önceki mesajların prompt'a nasıl enjekte edileceğini belirtir. 
Prompt'umuzda chat_history adında bir MessagesPlaceholder olduğu için, bu değeri buna uygun olarak ayarlıyoruz.

output_messages_key: (Zinciriniz birden fazla çıktı üretiyorsa) Hangi çıktının geçmiş olarak saklanacağını belirtir. 
Bu, input_messages_key'in tersidir.

RunnableWithMessageHistory: Bu sınıf, mesajların otomatik olarak yönetilmesini sağlayarak,
zincirinizi daha verimli hale getirir. --> mesajların manuel olarak yönetilmesi ihtiyacını ortadan kaldırır.
...
while True:
    ...
    demo_ephemeral_chat_history_for_chain.add_ai_message(result)
